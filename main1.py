...
mypipline = Pipeline([('read_data', ReadData(data_dir='./Data', file_name='cancer2.csv'))
                      #    ,
...
dense_features1 = ['Age', 'BMI', 'Total_blood_volume_litres_Nadlerformula']
...
# x_train_dense_features2=string_float(data_train[dense_features2]).values
# x_test_dense_features2=string_float(data_test[dense_features2]).values
# N = 5000
# n_components = 2
# tsne = TSNE(n_components=n_components, perplexity=50, n_iter=N, init='pca',random_state=0,verbose=1, method='exact')
# tsne_results_train = tsne.fit_transform(x_train_dense_features2)
# tsne_results_test = tsne.fit_transform(x_test_dense_features2)
#
# data_dense_train=np.concatenate([string_float(data_train[dense_features1]).values,tsne_results_train],axis=1)
# data_dense_test=np.concatenate([string_float(data_test[dense_features1]).values,tsne_results_test],axis=1)
...
scaler = MinMaxScaler((0, 1)).fit(string_float(data_train[sequential_features]).values)
y_train_sequential = scaler.transform(string_float(data_train[sequential_features]).values)
y_train_class
...
if test_mode == True:
    ...
    yy = tf.concat(output_sequence_, 1)
    ...
    myplot(scaler.inverse_transform(y_test_sequential), scaler.inverse_transform(yy), 5, 5,
           'tsne_new_750_20k_CRT1lymphocyte_absolute_count_KuL')
...
loss_pred(y_test_sequential,yy)
plt.plot(train_loss)
loss_pred(scaler.inverse_transform(y_test_sequential),scaler.inverse_transform(yy))
